...
(() => {
  ...
  function draw(){
    ...
    // Effetti: floating text
    for(const ef of state.effects){
      const a = Math.max(0, Math.min(1, ef.ttl/600));
      ctx.globalAlpha = a;
    ...
    if(state.flashT>0){
      const a = Math.min(0.35, state.flashT/1200);
  ...
  function step(dt){ // dt in secondi
    ...
    // Update effetti
    for(const ef of state.effects){ ef.ttl -= dt*1000; ef.y -= dt*60; }
    state.effects = state.effects.filter(e=>e.ttl>0);
    if(state.flashT>0) state.flashT -= dt*1000;
